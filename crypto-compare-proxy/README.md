...
* `docker run -p 8080:8080 crypto-compare-proxy:latest`
...
* `docker rmi crypto-compare-proxy -f`
...
*  in `pom.xml` uncomment `<!--packaging>war</packaging-->`

*  in `com.cc.proxy.CryptoCompareProxyApp` uncomment `/*extends SpringBootServletInitializer*/` and add `import org.springframework.boot.web.servlet.support.SpringBootServletInitializer;` to the imported packages.
...
* generated with: `mvn -N io.takari:maven:0.7.7:wrapper -Dmaven=3.8.6`

* run with: `./mvnw clean package spring-boot:run -DskipTests`
